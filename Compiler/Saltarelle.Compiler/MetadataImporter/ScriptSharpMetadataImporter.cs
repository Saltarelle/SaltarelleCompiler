	...
	public class ScriptSharpMetadataImporter : IMetadataImporter, IScriptSharpMetadataImporter {
		...
		private void ProcessMethod(IMethod method, string preferredName, bool nameSpecified, Dictionary<string, bool> usedNames) {
			...
			if (ioa != null) {
				...
				bool implementsInterfaceMember = method.IsExplicitInterfaceImplementation || method.ImplementedInterfaceMembers.Any(m => GetMethodSemantics((IMethod)m.MemberDefinition).Type != MethodScriptSemantics.ImplType.NotUsableFromScript);
		...
		public bool IsRealType(ITypeDefinition t) {
			return GetTypeSemanticsInternal(t).IsRealType;
		}
