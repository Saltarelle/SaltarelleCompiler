...
Function EmbedReferences([String] $Dir, [String] $Executable) {
}
...
Task Build-Compiler -Depends Clean, Generate-VersionInfo, Fix-AntlrLocalization {
	...
	#Exec { & "$buildtoolsDir\ilmerge.exe" /ndebug "/targetplatform:v4,C:\Windows\Microsoft.NET\Framework\v4.0.30319" "/out:$outDir\sc.exe" "/keyfile:$baseDir\Saltarelle.snk" "$exedir\sc.exe" "$exedir\Saltarelle.Compiler.JSModel.dll" "$exedir\Saltarelle.Compiler.dll" "$exedir\ICSharpCode.NRefactory.dll" "$exedir\ICSharpCode.NRefactory.CSharp.dll" "$exedir\Mono.Cecil.dll" "$exedir\JavaScriptParser.dll" "$exedir\Antlr3.Runtime.dll" "$exedir\Castle.Core.dll" "$exedir\Castle.Windsor.dll" }
	EmbedReferences "$exedir", "sc.exe"
	Exec { & "$buildtoolsDir\ilmerge.exe" /ndebug "/targetplatform:v4,C:\Windows\Microsoft.NET\Framework\v4.0.30319" "/out:$outDir\SCTask.dll" "/keyfile:$baseDir\Saltarelle.snk" "$taskdir\SCTask.dll" "$taskdir\Saltarelle.Compiler.JSModel.dll" "$taskdir\Saltarelle.Compiler.dll" "$taskdir\ICSharpCode.NRefactory.dll" "$taskdir\ICSharpCode.NRefactory.CSharp.dll" "$taskdir\Mono.Cecil.dll" "$exedir\JavaScriptParser.dll" "$exedir\Antlr3.Runtime.dll" "$exedir\Castle.Core.dll" "$exedir\Castle.Windsor.dll" }
