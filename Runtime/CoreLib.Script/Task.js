...
var ss_Task = ss.Task = ss.mkType(ss, 'ss.Task',
	function#? DEBUG Task$##(action, state) {
		this._action = action;
		this._state = state;
		this.exception = null;
		this.status = 0;
	...
	{
		delay: function#? DEBUG Task$delay##(delay) {
			var tcs = new ss_TaskCompletionSource();
			setTimeout(function() {
				tcs.setResult(0);
			}, delay);
		...
		fromResult: function#? DEBUG Task$fromResult##(result) {
			var t = new ss_Task();
			t.status = 5;
			t._result = result;
			return t;
		},
		run: function#? DEBUG Task$run##(f) {
			var tcs = new ss_TaskCompletionSource();
			setTimeout(function() {
				try {
					tcs.setResult(f());
		...
		whenAll: function#? DEBUG Task$whenAll##(tasks) {
			var tcs = new ss_TaskCompletionSource();
			if (tasks.length === 0) {
				tcs.setResult([]);
			}
		...
		whenAny: function#? DEBUG Task$whenAny##(tasks) {
			if (!tasks.length)
		...
		fromDoneCallback: function#? DEBUG Task$fromDoneCallback##(t, i, m) {
		...
		fromPromise: function#? DEBUG Task$fromPromise##(p, f) {
			var tcs = new ss_TaskCompletionSource();
			if (typeof(f) === 'number')
		...
		fromNode: function #? DEBUG Task$fromNode##(t, f, m) {
			var tcs = new ss_TaskCompletionSource(), args;
