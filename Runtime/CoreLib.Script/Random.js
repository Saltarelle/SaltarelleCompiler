...
ss_Random.prototype = {
	...
	next: function#? DEBUG Random$next##(min, max) {
	    if(min === undefined && max === undefined)
	        return parseInt(this.sample() * 2147483647);
	                                                   
	    var difference = max - min;
	    return parseInt((this.sample() * difference) + min);
	},

    nextBytes: function#? DEBUG Random$nextBytes##(bytes) {
        for (var i = 0; i < bytes.length; i++)
            bytes[i] = parseInt(this.sample() * 255);
	},
