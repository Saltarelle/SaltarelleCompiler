...
namespace CoreLibTests {
	...
	public class TypeSystemLanguageSupportTests {
		...
		public void GetTypeWorksOnObjects() {
			Action a = () => {};
			Assert.AreEqual(new C1().GetType().FullName, "CoreLibTests.TypeSystemLanguageSupportTests$C1");
			Assert.AreEqual(new C2<int>().GetType().FullName, "CoreLibTests.TypeSystemLanguageSupportTests$C2$1[ss.Int32]");
			Assert.AreEqual(new C2<string>().GetType().FullName, "CoreLibTests.TypeSystemLanguageSupportTests$C2$1[String]");
