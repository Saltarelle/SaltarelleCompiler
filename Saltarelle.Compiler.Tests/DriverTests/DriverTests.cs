...
public class C1 {
	public void M() {
		...
		[Test]
		public void TheAssemblyNameIsCorrectAndCanBeSpecified() {
			UsingFiles(() => {
				File.WriteAllText(Path.GetFullPath("File.cs"), @"class Class1 { public void M() {} }");
				var options = new CompilerOptions {
					References         = { new Reference(Common.SSMscorlibPath) },
			...
			UsingFiles(() => {
				...
				Assert.That(File.Exists(Path.GetFullPath("Test.xml")), Is.True);
				string doc = File.ReadAllText(Path.GetFullPath("Test.xml"));
				Assert.That(XDocument.Parse(doc).XPathSelectElement("/doc/assembly/name").Value, Is.EqualTo("MyAssemblyName"));

				var asm = AssemblyDefinition.ReadAssembly(Path.GetFullPath("Test.dll"));
				Assert.That(asm.Name.Name, Is.EqualTo("MyAssemblyName"));
