...
@"function() {
	this.$MyProperty = 0;
		...
		public void InstanceAutoPropertyAccessorsImplementedAsStaticMethodsAreCorrectlyCompiled() {
            Compile(new[] { "using System; class C { public int MyProperty { get; set; } }" }, namingConvention: new MockNamingConventionResolver { GetPropertyImplementation = p => PropertyImplOptions.GetAndSetMethods(MethodImplOptions.StaticMethodWithThisAsFirstArgument("get_" + p.Name), MethodImplOptions.StaticMethodWithThisAsFirstArgument("set_" + p.Name)) });
