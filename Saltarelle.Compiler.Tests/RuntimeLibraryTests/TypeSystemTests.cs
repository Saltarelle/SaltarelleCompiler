...
public class C {
	public static bool[] M() {
		return new[] { typeof(C1<>).IsGenericTypeDefinition, typeof(C1<int>).IsGenericTypeDefinition, typeof(C2).IsGenericTypeDefinition, typeof(I1<>).IsGenericTypeDefinition, typeof(I1<int>).IsGenericTypeDefinition, typeof(I2).IsGenericTypeDefinition };
	}
}", "C.M");
			Assert.That(result, Is.EqualTo(new[] { true, false, false, true, false, false }));
		}
...
public class C {
	public static int[] M() {
		return new[] { typeof(C1<>).GenericParameterCount, typeof(C1<int>).GenericParameterCount, typeof(C2).GenericParameterCount, typeof(I1<>).GenericParameterCount, typeof(I1<int>).GenericParameterCount, typeof(I2).GenericParameterCount };
	}
}", "C.M");
			Assert.That(result, Is.EqualTo(new[] { 1, 0, 0, 1, 0, 0 }));
		}
...
public class C {
	...
	public static string[][] M() {
		return new[] { F(typeof(C1<>).GetGenericArguments()), F(typeof(C1<int>).GetGenericArguments()), F(typeof(C2).GetGenericArguments()), F(typeof(I1<>).GetGenericArguments()), F(typeof(I1<string>).GetGenericArguments()), F(typeof(I2).GetGenericArguments()) };
	}
}", "C.M");
			var l = ((IEnumerable)result).Cast<IList>().ToList();
			Assert.That(l.Select(x => x != null).ToList(), Is.EqualTo(new[] { false, true, false, false, true, false }));
...
public class C {
	private static string Name(System.Type t) { return t != null ? t.FullName : null; }
	public static string[] M() {
		return new[] { Name(typeof(C1<>).GetGenericTypeDefinition()), Name(typeof(C1<int>).GetGenericTypeDefinition()), Name(typeof(C2).GetGenericTypeDefinition()), Name(typeof(I1<>).GetGenericTypeDefinition()), Name(typeof(I1<string>).GetGenericTypeDefinition()), Name(typeof(I2).GetGenericTypeDefinition()) };
	}
}", "C.M");
			Assert.That(result, Is.EqualTo(new[] { null, "C1$1", null, null, "I1$1", null }));
		}
	}
}
